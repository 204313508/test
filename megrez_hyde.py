"""test_megrez_hyde.py
在MMLU测试集上本地运行 Megrez-3B-Omni，并保存预测结果（使用Hypothetical Document Embeddings, HyDE）。

此脚本假设模型权重已下载到指定路径。

用法::

    python test_megrez_hyde.py --input data/mmlu/test.jsonl --output result.jsonl \
...
def process_file(model, input_path: Path, output_path: Path):
    ...
    with input_path.open("r", encoding="utf-8") as fin, output_path.open("a", encoding="utf-8") as fout:
        for idx, line in enumerate(fin):
            ...
            fout.flush()
            print(f"Processed {idx}: {record.get('question')[:30]}... -> {llm_answer} (Time: {elapsed:.3f}s)")
...
def main():
    parser = argparse.ArgumentParser(description="使用 Megrez-3B-Omni 运行 MMLU 测试集（HyDE 两阶段推理）")
    parser.add_argument("--input", type=str, default="data/mmlu/test.jsonl", help="MMLU测试集路径")
    parser.add_argument("--output", type=str, default="megrez_hyde_result.jsonl", help="输出结果路径")
